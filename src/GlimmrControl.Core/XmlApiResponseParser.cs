    ...
    class XmlApiResponseParser
    {
        public static XmlApiResponse ParseApiResponse(string xml)
        {
            ...
            try
            {
                ...
                if (resp.Name == null) resp.Name = xe.Element("desc")?.Value; //try legacy XML element name (pre Glimmr 0.6.0)
                if (resp.Name == null) return null; //if we return at this point, parsing was unsuccessful (server likely not Glimmr device)

                string bri_s = xe.Element("ac")?.Value;
                if (bri_s == null) bri_s = xe.Element("act")?.Value; //try legacy XML element name (pre Glimmr 0.6.0)
                if (bri_s != null)
                {
                    int bri = 0;
